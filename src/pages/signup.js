...
export default function signup(){
    ...
    const handleSubmit = (event) => {
      ...
      if (!inputs.fname || !thaiAndEnglishRegex.test(inputs.fname)) {
        setErrors((prevErrors) => ({ ...prevErrors, fname: "กรุณากรอกชื่อที่ถูกต้อง" }));
        return;
      }
  
      // Validate lname
      if (!inputs.lname || !thaiAndEnglishRegex.test(inputs.lname)) {
        setErrors((prevErrors) => ({ ...prevErrors, lname: "กรุณากรอกนามสกุลที่ถูกต้อง" }));
        return;
      }
  
      // Validate username
      if (!inputs.username || !usernameRegex.test(inputs.username)) {
        setErrors((prevErrors) => ({ ...prevErrors, username: "กรุณากรอกชื่อผู้ใช้ที่ถูกต้อง" }));
        return;
      }
  
      // Validate password
      if (!inputs.password || !passwordRegex.test(inputs.password)) {
        setErrors((prevErrors) => ({ ...prevErrors, password: "กรุณากรอกรหัสผ่านที่ถูกต้อง" }));
        return;
      }
              ...
              <div className="text-gray-950 text-center pt-3 text-2xl max-[650px]:w-full">
                ...
                <div className="mt-[2rem]">
                  ...
                  <input
                    ...
                    value={inputs.fname || ""}
                    className={`bg-[#01BAB5] text-[#fff] rounded-xl border-none outline-none ${errors.fname && "outline-red"}`}
                  />
                ...
                <div className="mt-[2rem]">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    value={inputs.lname || ""}
                    className={`bg-[#01BAB5] text-[#fff] rounded-xl border-none outline-none ${errors.lname && "outline-red"}`}
                  />
                ...
                <div className="mt-[2rem]">
                  ...
                  <input
                    ...
                    className={`bg-[#01BAB5] text-[#fff] rounded-xl border-none outline-none ${errors.username && "outline-red"}`}
                  />
                ...
                <div className="mt-[2rem]">
                  ...
                  <input
                    ...
                    className={`bg-[#01BAB5] text-[#fff] rounded-xl border-none outline-none ${errors.password && "outline-red"}`}
                  />
